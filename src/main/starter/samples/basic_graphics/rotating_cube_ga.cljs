(ns starter.samples.basic-graphics.rotating-cube-ga
  "Rotating cube using Geometric Algebra with refactored API.

   This sample demonstrates:
   - Geometric Algebra (GA) for rotations (rotors, not matrices!)
   - Data-oriented scene description
   - Separation of concerns (Data → Calculations → Actions)
   - Shader DSL integration
   - Why GA is superior to matrices for graphics

   KEY INSIGHTS:
   - GA rotors have no gimbal lock
   - Rotors interpolate smoothly (great for animation)
   - Rotations happen IN a plane (bivector), not AROUND an axis
   - Same math works in 2D, 3D, 4D, nD
   - More intuitive geometric meaning"
